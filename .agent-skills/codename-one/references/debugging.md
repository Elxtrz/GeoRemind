...
| Command | Purpose |
| --- | --- |
| `stop in com.tijil.reminder.GeoReminder.runApp` | Breakpoint on a method entry. |
| `stop at com.tijil.reminder.LoginForm:42` | Breakpoint at source line 42. |
...
5. **Set the breakpoint, then continue**: at the `jdb >` prompt:
   ...
   ```
   stop at com.tijil.reminder.LoginForm:42
   cont
   ```
